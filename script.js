...
function ageCalculate() {
    let birthInputDate = new Date(document.getElementById("birth-date-input").value);
    let CurrentInputDate = new Date(document.getElementById("current-date-input").value);
    ...
    if (
        ...
        (birthDetails.birthMonth > currentDetails.currentMonth && birthDetails.birthYear == currentDetails.currentYear) ||
        (birthDetails.birthDate > currentDetails.currentDate && birthDetails.birthMonth == currentDetails.currentMonth && birthDetails.birthYear == currentDetails.currentYear)
    ) {
        alert("NOT BORN YET");
        return;
    }
    ...
    if(currentDetails.currentMonth >= birthDetails.birthMonth){
        ...
        calculated_year--;
        calculated_month = 12 + currentDetails.currentMonth - birthDetails.birthMonth 
    }
    ...
    if(currentDetails.currentDate >= birthDetails.birthDate){
        ...
        calculated_month--;
        let days = months[currentDetails.currentMonth - 1];
        calculated_date = days + currentDetails.currentDate - birthDetails.birthDate;
